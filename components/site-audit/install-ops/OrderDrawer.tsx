...
import { sbGet, sbPatch } from '../siteAuditShared';
import AssignSection from './AssignSection';
import { Chip, MapLink, Note } from './ui';
import {
  AUTO_STATUSES, STATUS, dstr, fmtDate, rollHintText, skuQtyField, slotsForWp, syncParentStatus, today, totalRolls, emptySkuRow, fmtLogLocal, opsCallDue,
} from './shared';
import { genInstallPDFSM } from './pdf';
import type { InstallOrder, Installer, ServiceSkuRow, SlotDef, Subjob } from './types';
...
export default function OrderDrawer({ order: o, installers, slotsFl, slotsWp, attribution, onClose, onOpenOrder, onOpenRect, reload, reloadWithDeleted, toast }: Props) {
  const [draft, setDraft] = useState<DraftState>(() => buildInitDraft(o));
  const [grpOn, setGrpOn] = useState(() => { const d = buildInitDraft(o); return { flooring: d.flooring.length > 0, wallpaper: d.wallpaper.length > 0 }; });
  const [newDeliv, setNewDeliv] = useState(o.deliveryDate || '');
  const [fuDate, setFuDate] = useState((o.service && o.service.follow_up_date) || '');

  function updateDraftField(grp: 'flooring' | 'wallpaper', i: number, f: keyof ServiceSkuRow, v: string) {
    setDraft((d) => ({ ...d, [grp]: d[grp].map((r, ri) => (ri === i ? { ...r, [f]: v } : r)) }));
  }
  function delDraftRow(grp: 'flooring' | 'wallpaper', i: number) {
    setDraft((d) => ({ ...d, [grp]: d[grp].filter((_, ri) => ri !== i) }));
  }
  function addDraftRow(grp: 'flooring' | 'wallpaper') {
  ...
  const setStatus = async (st: string) => {
    ...
    if (st === 'completed' && o.subjobs && o.subjobs.length) {
      ...
      if (noCard.length) {
        const names = noCard.map((sj) => (sj.type === 'wallpaper' ? 'Wallpaper' : 'Flooring')).join(', ');
  ...
  const createService = async () => {
    const fl = draft.flooring.filter((r) => r.sku || r.name), wp = draft.wallpaper.filter((r) => r.sku || r.name);
    if (!fl.length && !wp.length) { toast('Add at least one SKU'); return; }
    const nextService = { ...(o.service || {}), flooring: fl, wallpaper: wp };
    const nextSubjobs: Subjob[] = [];
    if (fl.length) nextSubjobs.push({ id: 'sj_fl', type: 'flooring', items: fl, date: null, slot: null, installer: null, installer_email: null, assignments: [], status: 'created' });
    if (wp.length) nextSubjobs.push({ id: 'sj_wp', type: 'wallpaper', items: wp, date: null, slot: null, installer: null, installer_email: null, assignments: [], status: 'created' });
  ...
  const saveService = async () => {
    const fl = draft.flooring.filter((r) => r.sku || r.name), wp = draft.wallpaper.filter((r) => r.sku || r.name);
    const flSj = o.subjobs && o.subjobs.find((s) => s.id === 'sj_fl');
    const wpSj = o.subjobs && o.subjobs.find((s) => s.id === 'sj_wp');
    const logAdds: Array<{ t: string; d: string; by: 'manual'; who: string }> = [];
    if (!fl.length && flSj && flSj.status !== 'created') {
      if (!window.confirm('The flooring sub-job is currently ' + (STATUS[flSj.status] || { l: flSj.status }).l + '. Removing it will cancel the installer\'s assignment. Proceed?')) return;
      logAdds.push({ t: 'Flooring sub-job removed (was ' + flSj.status + ' — SM correction)', d: new Date().toISOString(), by: 'manual', who: attribution });
    }
    if (!wp.length && wpSj && wpSj.status !== 'created') {
      if (!window.confirm('The wallpaper sub-job is currently ' + (STATUS[wpSj.status] || { l: wpSj.status }).l + '. Removing it will cancel the installer\'s assignment. Proceed?')) return;
      logAdds.push({ t: 'Wallpaper sub-job removed (was ' + wpSj.status + ' — SM correction)', d: new Date().toISOString(), by: 'manual', who: attribution });
    }
    const nextService = { ...(o.service || {}), flooring: fl, wallpaper: wp };
    let nextSubjobs = o.subjobs ? [...o.subjobs] : null;
    if (nextSubjobs) {
      if (fl.length && flSj) nextSubjobs = nextSubjobs.map((s) => (s.id === 'sj_fl' ? { ...s, items: fl } : s));
      else if (fl.length && !flSj) nextSubjobs.push({ id: 'sj_fl', type: 'flooring', items: fl, date: null, slot: null, installer: null, installer_email: null, assignments: [], status: 'created' });
      else if (!fl.length && flSj) nextSubjobs = nextSubjobs.filter((s) => s.id !== 'sj_fl');
      if (wp.length && wpSj) nextSubjobs = nextSubjobs.map((s) => (s.id === 'sj_wp' ? { ...s, items: wp } : s));
      else if (wp.length && !wpSj) nextSubjobs.push({ id: 'sj_wp', type: 'wallpaper', items: wp, date: null, slot: null, installer: null, installer_email: null, assignments: [], status: 'created' });
      else if (!wp.length && wpSj) nextSubjobs = nextSubjobs.filter((s) => s.id !== 'sj_wp');
    }
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        ...
        {o.service && o.status !== 'deliv_ontime' ? (
        ...
        {o.subjobs ? (
          <Section title="Installation sub-jobs">
            <Note tone="blue">Flooring and wallpaper are scheduled &amp; tracked separately. Each can have multiple installers. Order completes only when all sub-jobs are done.</Note>
            {o.subjobs.map((sj) => {
              ...
              return (
                <div className="rounded-lg border border-gray-200 p-3.5 mb-3" key={sj.id}>
                  <div className="flex items-center gap-2 mb-2.5 flex-wrap">
                    <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold ${sj.type === 'wallpaper' ? 'bg-purple-100 text-purple-700' : 'bg-yellow-50 text-yellow-800'}`}>{sj.type === 'wallpaper' ? 'WALLPAPER' : 'FLOORING'}</span>
                  ...
                  {sj.type === 'wallpaper'
                    ...
                    : <div className="text-[12px] text-green-700 mb-2.5">Flooring — 1 full day per installer visit</div>}
                  {sj.status === 'completed' ? <DownloadJobCardBtn order={o} sjId={sj.id} installers={installers} toast={toast} /> : null}
                  <AssignSection order={o} subjob={sj} installers={installers} slotsFl={slotsFl} slotsWp={slotsWp} attribution={attribution} reload={reload} toast={toast} onOpenOrder={onOpenOrder} />
                </div>
              );
            })}
          </Section>
...
function SkuGroup({
  ...
  grp: 'flooring' | 'wallpaper'; label: string; draft: DraftState; grpOn: { flooring: boolean; wallpaper: boolean };
  toggleGrp: (g: 'flooring' | 'wallpaper') => void;
  updateDraftField: (g: 'flooring' | 'wallpaper', i: number, f: keyof ServiceSkuRow, v: string) => void;
  delDraftRow: (g: 'flooring' | 'wallpaper', i: number) => void;
  addDraftRow: (g: 'flooring' | 'wallpaper') => void;
}) {
  const on = grpOn[grp];
  return (
    <div className="mb-3">
      ...
      {on ? (
        <div>
          ...
          <button className="border border-dashed border-blue-500 bg-white text-blue-600 rounded-md py-1.5 text-[12.5px] font-semibold w-full" onClick={() => addDraftRow(grp)}>+ Add {grp} SKU</button>
...
function SkuRow({
  ...
  r: ServiceSkuRow; grp: 'flooring' | 'wallpaper'; i: number;
  onField: (g: 'flooring' | 'wallpaper', i: number, f: keyof ServiceSkuRow, v: string) => void;
