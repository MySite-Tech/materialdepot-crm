...
import {
  dateRange, fmtDate, installerById, slotLabel, slotsForWp, syncParentStatus, totalRolls, dstr, today,
} from './shared';
...
export default function AssignSection({ order: o, subjob: sj, installers, slotsFl, slotsWp, attribution, reload, toast, onOpenOrder }: Props) {
  const [, setTick] = useState(0);
  const redraw = () => setTick((x) => x + 1);

  const pool = useMemo(() => installers.filter((i) => i.type === sj.type), [installers, sj.type]);
  const minDate = (o.deliveryDate && o.deliveryDate >= dstr(today)) ? o.deliveryDate : dstr(today);
  ...
  const bookSlot = async () => {
    ...
    if (wasResched) { nextSj.assignments = []; assignsRef.current = []; }
  ...
  const clearSlot = async () => {
    ...
    const nextSj: Subjob = { ...sj, date: null, slot: null, assignments: [], status: 'created' };
    const nextSubjobs = replaceSubjob(nextSj);
    const nextStatus = syncParentStatus(nextSubjobs, o.status);
  ...
  const saveAssign = async () => {
    ...
    const nextLog = [...o.log, { t: logLabel + ': ' + valid.map((a) => a.installer_name).join(', '), d: new Date().toISOString(), by: 'manual' as const, who: attribution }];
    if (o.id) await sbPatch('install_orders', String(o.id), { status: nextStatus, subjobs: nextSubjobs, log: nextLog });
    await reload();
          ...
          {slotDone ? (
            <div className="rounded-lg border border-gray-200">
              ...
              <div className="px-3 py-2.5">
                ...
                <div className="border border-dashed border-blue-500 text-blue-600 bg-white rounded-md text-center py-2 text-[12.5px] font-semibold cursor-pointer" onClick={addAssign}>＋ Add another installer</div>
                <button className="bg-[#1F3A5F] text-white w-full mt-1 py-2 rounded-md text-[13px] font-semibold" onClick={saveAssign}>Save assignment</button>
              </div>
            </div>
          ) : null}
        </>
