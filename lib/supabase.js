...
export async function fetchLeads() {
  ...
  // Deduplicate by ID — keep latest (first occurrence since sorted desc by created_at)
  const seen = new Set();
...
export async function appendRemarkToLead(leadId, remark) {
...
export async function upsertLeads(leads) {
  // Deduplicate incoming list by ID — keep last occurrence
  const deduped = Object.values(Object.fromEntries(leads.map((l) => [l.id, l])));
  const incomingRows = deduped.map(toRow);

  // Fetch existing rows from DB for these IDs so we can smart-merge
  ...
  const existingMap = Object.fromEntries((existingData || []).map(r => [r.id, r]));
  const mergedRows = incomingRows.map(row =>
    existingMap[row.id] ? mergeRow(existingMap[row.id], row) : row
  );
