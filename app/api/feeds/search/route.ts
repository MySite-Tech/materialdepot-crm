...
export async function POST(request: NextRequest) {
  ...
  const sort = searchParams.get("sort") ?? "performedAt,desc";
  const body = await request.json();

  cleanup();
  const cacheKey = `feeds:${page}:${size}:${sort}:${JSON.stringify(body)}`;
  const cached = getCached(cacheKey);
  if (cached) return Response.json(cached);

  try {
    const res = await rateLimitedFetch(
      `${KYLAS_API_BASE}/feeds/search?page=${page}&size=${size}&sort=${encodeURIComponent(sort)}`,
      {
        method: "POST",
